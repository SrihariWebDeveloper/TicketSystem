...
type AuthContextValue = {
  ...
  updateUser: (email: string, changes: Partial<AuthUser>) => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = (email: string, password: string, remember: boolean) => {
    ...
    if (!user) return "The email or password is incorrect.";
    if (user.status !== "Active")
      return "This account is inactive. Contact an administrator.";
    setCurrentUser(user);
    if (!remember) sessionStorage.setItem("supporthub_session", "1");
    return null;
  };
  const logout = () => setCurrentUser(null);
  const createUser = (user: AuthUser) => setUsers((items) => [...items, user]);
  const updateUser = (email: string, changes: Partial<AuthUser>) => {
    setUsers((items) =>
      items.map((item) =>
        item.email === email ? { ...item, ...changes } : item,
      ),
    );
    setCurrentUser((user) =>
